...
def backup(base_dir: str, slug: str):
    ...
    # 备份核心文件
    for fname in ['memory.md', 'persona.md', 'emotion_profile.md', 'long_term_memory.md', 'SKILL.md', 'meta.json']:
...
def main():
    parser = argparse.ArgumentParser(description='灵犀版本管理器')
    parser.add_argument('--action', required=True, choices=['backup', 'rollback', 'list'])
    parser.add_argument('--slug', required=True, help='陪伴者代号')
