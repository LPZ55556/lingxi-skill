...
灵犀接受纯文本格式的聊天记录。你不需要任何专门的导出工具，下面是几种获取聊天文本的方式。

---

## 方式一：直接复制粘贴（最简单）

打开跟 ta 的聊天窗口，手动选中消息，复制，粘贴到一个 txt 文件里。
...
如果聊天记录太长不想复制，可以直接截图上传。灵犀支持图片输入，Claude 会从截图中提取对话内容。

也可以什么都不导，纯靠口述：描述 ta 的说话方式、性格特点、你们之间的故事。仅凭描述也能生成 Skill，只是还原度不如真实记录高。
...
1. **日常互动** — 最能体现真实的相处模式
2. **关键时刻** — 开心/难过/冲突时的反应比日常更真实
3. **深夜对话** — 情绪浓度高，容易暴露真实性格
4. **特殊记忆** — 纪念日、旅行、重大事件相关对话
